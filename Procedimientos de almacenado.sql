...
BEGIN
  INSERT INTO EMPRESA VALUES (SECUENCIA_ID_EMPRESA.NEXTVAL,NOMBRE_EMPRESA_PARAMETRO,DESCRIPCION_PARAMETRO,ID_RUBRO_EMPRESA_PARAMETRO,
  ...
  INSERT INTO CONTRATO_EMPRESA VALUES (SECUENCIA_ID_CONTRATO_EMPRESA.NEXTVAL,DESCRIPCION_CONTRATO_PARAMETRO,FECHA_INICIO_PARAMETRO,ID_RESCATADO,ESTADO_CONTRATO_PARAMETRO,FECHA_TERMINO_PARAMETRO);
END SP_INSERTAR_EMPRESA;

--LLamando al procedimiento.
exec SP_INSERTAR_EMPRESA('PRUEBA STORE PROCEDURE','DESCRIPCION PRUEBA',2,'701223457',1,'\IMG\LOGO4','PRUEBA@PRUEBA.CL','PRUEBA_RAZON_SOCIAL','EJERCITO 215','DESCRIPCION CONTRATO PRUEBA','12/09/2019',1,SYSDATE);
...
--LISTAR EMPRESA 4.0
SET SERVEROUTPUT ON;
CREATE OR REPLACE PROCEDURE SP_LISTAR_EMPRESAS(LISTA_EMPRESA OUT SYS_REFCURSOR)
AS
BEGIN
      OPEN LISTA_EMPRESA FOR
        SELECT * FROM EMPRESA;
END;
...
--PROCEDIMIENTO 5 (INSERTAR PROFESIONAL)
SET SERVEROUTPUT ON;
...
--PROCEDIMIENTO 6 (ACTUALIZAR PROFESIONAL)
SET SERVEROUTPUT ON;
...
--PROCEDIMIENTO 7 (ELIMINAR PROFESIONAL (ELIMINACIÓN FISICA))
SET SERVEROUTPUT ON;
...
--PROCEDIMIENTO 7.1 (ELIMINAR PROFESIONAL (ELIMINACIÓN LOGICA))
SET SERVEROUTPUT ON;
...
--PROCEDIMIENTO 8.0 (LOGGIN)
SET SERVEROUTPUT ON;
CREATE OR REPLACE PROCEDURE SP_LOGGIN2
(--Parametros para llenar la tabla PROFESIONAL
USUARIO_PARAMETRO     IN USUARIO.NOMBRE_USUARIO%TYPE,
CLAVE_PARAMETRO       IN USUARIO.CLAVE%TYPE,
PERFIL OUT SYS_REFCURSOR)
IS
...
BEGIN
  ...
  INTO USUARIO_RESCATADO ,  CLAVE_RESCATADO 
  FROM USUARIO
  WHERE NOMBRE_USUARIO = USUARIO_PARAMETRO AND CLAVE = CLAVE_PARAMETRO;

    IF
        USUARIO_PARAMETRO = USUARIO_RESCATADO AND CLAVE_PARAMETRO = CLAVE_RESCATADO
    THEN
        OPEN PERFIL FOR 
          SELECT U.NOMBRE_USUARIO , U.CLAVE , U.ID_PERFIL , E.RAZON_SOCIAL , R.NOMBRE , R.RUT
...
--LLamando al procedimiento.
exec SP_LOGGIN2('CSANDOVAL','DILAN');
...
--PROCEDIMIENTO 9 (INSERTAR USUARIO)
SET SERVEROUTPUT ON;
CREATE OR REPLACE PROCEDURE SP_INSERTAR_USUARIO
(--Parametros para llenar la tabla USUARIO
CORREO_USUARIO_PARAMETRO IN USUARIO.NOMBRE_USUARIO%TYPE, 
CLAVE_PARAMETRO IN USUARIO.CLAVE%TYPE,
ID_PERFIL_PARAMETRO IN USUARIO.ID_PERFIL%TYPE,
ID_EMPRESA_PARAMETRO USUARIO.ID_EMPRESA%TYPE)
IS
BEGIN

  INSERT INTO USUARIO VALUES (SECUENCIA_ID_EMPRESA.NEXTVAL,CORREO_USUARIO_PARAMETRO,CLAVE_PARAMETRO,ID_PERFIL_PARAMETRO,ID_EMPRESA_PARAMETRO);
   
END SP_INSERTAR_USUARIO;

--LLamando al procedimiento.
EXEC SP_INSERTAR_USUARIO('OM.ROMERO@GMAIL.COM','ZWORLD7',2,26);
...
--SOLUCIÓN PARA EL RICARDO MILOS <3
SET SERVEROUTPUT ON;
CREATE OR REPLACE PROCEDURE SP_LISTAR_EMPRESAS
IS
ID_EMPRESA_PARAMETRO EMPRESA.ID_EMPRESA%TYPE;
NOMBRE_EMPRESA_PARAMETRO EMPRESA.NOMBRE%TYPE; 
LISTA_EMPRESA SYS_REFCURSOR;
BEGIN
      OPEN LISTA_EMPRESA FOR
        SELECT ID_EMPRESA , NOMBRE FROM EMPRESA;
        FETCH LISTA_EMPRESA INTO ID_EMPRESA_PARAMETRO, NOMBRE_EMPRESA_PARAMETRO;
        CLOSE LISTA_EMPRESA;
        DBMS_OUTPUT.PUT_LINE(ID_EMPRESA_PARAMETRO || NOMBRE_EMPRESA_PARAMETRO);
END;

EXEC SP_LISTAR_EMPRESAS;
